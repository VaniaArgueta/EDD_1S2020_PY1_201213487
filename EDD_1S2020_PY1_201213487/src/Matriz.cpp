        ...
        if(nuevo->x < nodo_y->getInterno()->x){
            ...
            while(pivote != NULL){
                if(nuevo->getX() < pivote->getX()){
                    ...
                    if(pivote->score > 1 && pivote->Letra==""){
                        pivote->Letra = nuevo->Letra;
                        //std::cout<<"entro a modificar letra"<<std::endl;
                    }
                //std::cout<<"entro a repetido"<<std::endl;
                    break;
                }else{
...
void Matriz::graficar_logico(){
    ...
    while(pivote != NULL){
        ...
        while(pivote_interno!= NULL){
            ...
            //***************************
            if(pivote_interno->score == 2){
                fprintf(doc,"\n\tnode[label = \" %s \nDOBLE\" fillcolor=\" azure2\" pos = \"%d,-%d!\" shape = box]\"i%d-%d\";",pivote_interno->getLetra().c_str(),posx,posy_interno,pivote_interno->getX(),pivote_interno->getY());

            }else if(pivote_interno->score == 3){
                fprintf(doc,"\n\tnode[label = \" %s \nTRIPLE\" fillcolor=\" azure2\" pos = \"%d,-%d!\" shape = box]\"i%d-%d\";",pivote_interno->getLetra().c_str(),posx,posy_interno,pivote_interno->getX(),pivote_interno->getY());

            }else{
                fprintf(doc,"\n\tnode[label = \" %s \" fillcolor=\" azure2\" pos = \"%d,-%d!\" shape = box]\"i%d-%d\";",pivote_interno->getLetra().c_str(),posx,posy_interno,pivote_interno->getX(),pivote_interno->getY());
...
void Matriz::generar_imagen(){
    FILE * doc = fopen("matriz_imagen.dot","w");
    fprintf(doc,"digraph G{ rankdir = TB\n");
    fprintf(doc,"node[shape = box,width=1,height=1,fillcolor=\"azure2\" color=\"black\" style=\"filled\"];\n");
    fprintf(doc, "edge[style = \"bold\"]; \n");

    fprintf(doc,"\n\ node[label = \"\" fillcolor=\" darkolivegreen1\" color=\"black\" pos = \"-1,1!\"]principal;");

    //graficar nodos cabecera
    //graficar nodos x
    int total_cabecerasx= this->filas->getUltimo_listaCabeceras()->getId();
        for(int i=0;i<= total_cabecerasx;i++){
            fprintf(doc,"\n\tnode[label = \"%d\" fillcolor=\" azure2\" color=\"black\" pos = \"%d,1!\" shape = box]x%d;",i,i,i);
        }
    //graficar nodos y
    int total_cabecerasy= this->columnas->getUltimo_listaCabeceras()->getId();
        for(int i=0;i<= total_cabecerasy;i++){
            fprintf(doc,"\n\tnode[label = \"%d\" fillcolor=\" azure2\" color=\"black\" pos = \"-1,-%d!\" shape = box]y%d;",i,i,i);
        }
    //graficar nodos internos
    nodo_cabecera * pivote = this->filas->getPrimero_lisctaCabeceras();
    while(pivote != NULL){
        nodo_internoMatriz * pivote_interno = pivote->getInterno();
        while (pivote_interno != NULL){
            fprintf(doc,"\n\tnode[label = \"\" fillcolor=\" %s\" color=\"black\" pos = \"%d,-%d!\" shape = box]\"%d-%d;\"",pivote_interno->getLetra().c_str(),pivote_interno->getX(),pivote_interno->getY(),pivote_interno->getX(),pivote_interno->getY());
            pivote_interno = pivote_interno->getDerecho();
        }
        pivote=pivote->getSiguiente();
    }
        //graficar nodos vacios
        for(int i=0;i<= total_cabecerasx; i++){
            for(int j=0;j<= total_cabecerasy; j++){
                fprintf(doc,"\n\tnode[label = \"\" fillcolor=\" white\" color=\"black\" pos = \"%d,-%d!\" shape = box]\"%d-%d;\"",i,j,i,j);
            }
        }
    ...
    fclose(doc);
    system("cd src/" );
    system( "neato -Tpng matriz_imagen.dot -o matriz_imagen.png");
    system("cd src/" );
    system("xdg-open matriz_imagen.png");
}
